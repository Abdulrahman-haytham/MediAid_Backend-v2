...
import {
  ...
  UsedMedicineItem,
  UsedMedicineHistory,
} from '../entities/usedMedicine.entity';
...
export class UsedMedicineService {
  constructor(
    @InjectRepository(UsedMedicine)
    private readonly usedRepo: Repository<UsedMedicine>,
    @InjectRepository(UsedMedicineItem)
    private readonly itemRepo: Repository<UsedMedicineItem>,
    @InjectRepository(UsedMedicineHistory)
    private readonly historyRepo: Repository<UsedMedicineHistory>,
    @InjectRepository(Product)
    private readonly productRepo: Repository<Product>,
  ) {}

  async addOrUpdate(user: User, dto: CreateUsedMedicineDto) {
    let doc = await this.usedRepo.findOne({
      where: { user: { id: user.id } },
      relations: ['items', 'items.product', 'items.history'],
    });
    if (!doc) {
      doc = this.usedRepo.create({ user, items: [] });
      doc = await this.usedRepo.save(doc);
    }
    const applyOne = async (input: {
      productId: string;
      dosage: string;
      frequency: string;
      startDate: string;
      ...
      const items = await this.itemRepo.find({
        where: { usedMedicine: { id: doc.id } },
        relations: ['usedMedicine', 'product', 'history'],
      });
      ...
      if (existing) {
        ...
        await this.historyRepo.save(
          this.historyRepo.create({
            item: existing,
            updatedAt: new Date(),
            changes: 'Updated dosage/frequency',
          }),
        );
        ...
        await this.historyRepo.save(
          this.historyRepo.create({
            item: saved,
            updatedAt: new Date(),
            changes: 'Created medicine entry',
          }),
        );
    ...
    if (dto.medicines && dto.medicines.length > 0) {
      ...
      const full = await this.usedRepo.findOne({
        where: { id: doc.id },
        relations: ['items', 'items.product', 'items.history'],
      });
  ...
  async updateDetails(user: User, itemId: string, dto: UpdateUsedMedicineDto) {
    const doc = await this.usedRepo.findOne({
      where: { user: { id: user.id } },
    });
    if (!doc) throw new NotFoundException('No medicines found for this user');
    const item = await this.itemRepo.findOne({
      where: { id: itemId, usedMedicine: { id: doc.id } },
      relations: ['history'],
    });
