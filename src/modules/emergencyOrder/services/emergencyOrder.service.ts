...
import { EmergencyOrder, EmergencyOrderStatus, EmergencyOrderPriority, EmergencyOrderResponse, PharmacyResponseStatus } from '../entities/emergencyOrder.entity';
...
export class EmergencyOrderService {
  ...
  async createSmartOrder(user: User, dto: CreateEmergencyOrderDto): Promise<EmergencyOrder> {
    ...
    if (!product) {
      throw new NotFoundException(`Product "${requestedMedicine}" not found in our system.`);
    }
    ...
    const pharmacies = await this.pharmacyRepo
      ...
      .leftJoinAndSelect('pharmacy.medicines', 'pm', 'pm.productId = :productId', { productId: product.id })
      .addSelect(
        `(
    ...
    for (const item of pharmacies.entities) {
       ...
       const distanceScore = Math.max(0, ORDER_CONSTANTS.SCORING.DISTANCE_WEIGHT - (distance / ORDER_CONSTANTS.SCORING.BASE_DISTANCE_FACTOR) * ORDER_CONSTANTS.SCORING.DISTANCE_WEIGHT);
       const ratingScore = (item.averageRating || 0) / ORDER_CONSTANTS.SCORING.MAX_RATING * ORDER_CONSTANTS.SCORING.RATING_WEIGHT;
       const availabilityScore = hasProduct ? ORDER_CONSTANTS.SCORING.AVAILABILITY_WEIGHT : 0;
    ...
    if (topPharmacies.length === 0) {
      throw new NotFoundException('Unfortunately, no nearby pharmacies currently have this product in stock.');
    }
    ...
    const order = this.orderRepo.create({
      ...
      responseTimeout: new Date(Date.now() + (responseTimeoutInMinutes || ORDER_CONSTANTS.DEFAULT_RESPONSE_TIMEOUT_MINUTES) * 60 * 1000),
  ...
  async findOrderById(id: string): Promise<EmergencyOrder> {
    const order = await this.orderRepo.findOne({
      where: { id },
      relations: ['user', 'acceptedPharmacy', 'responses', 'responses.pharmacy', 'targettedPharmacies'],
    });
  ...
  async respondToOrder(user: User, orderId: string, dto: RespondToEmergencyOrderDto): Promise<EmergencyOrder> {
    const pharmacy = await this.pharmacyRepo.findOne({ where: { user: { id: user.id } } });
    if (!pharmacy) {
      throw new ForbiddenException('User is not associated with any pharmacy.');
    }
    ...
    const isTargeted = order.targettedPharmacies.some(p => p.id === pharmacy.id);
    if (!isTargeted) {
  ...
  async fulfillOrder(user: User, orderId: string): Promise<EmergencyOrder> {
    ...
    if (order.user.id !== user.id) {
        ...
        throw new ForbiddenException('Only the order creator can mark it as fulfilled.');
    }

    if (order.status !== EmergencyOrderStatus.ACCEPTED) {
        throw new BadRequestException('Order must be accepted before it can be fulfilled.');
    }
  ...
  async processOrderTimeouts(): Promise<void> {
    const expiredOrders = await this.orderRepo.createQueryBuilder('order')
